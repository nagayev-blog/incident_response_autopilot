...
<!-- DEMO: добавить GIF экрана — streamlit run ui/app.py → CRITICAL пресет → screenrecord 30 сек -->
...
Инженер физически подтверждает каждый план через HITL-паузу; без подтверждения
...
¹ Один выброс: 83 сек (postmortem занял 67 сек под нагрузкой LLM API) — реальная
задержка, не скрытая. Подробнее: [Engineering Notes](#engineering-notes).
...
это риск. Инженер должен явно взять ответственность. HITL здесь — не UX-фича,
...
3. **HITL требует паузы в известной точке.** Если граф emergent, непонятно,
...
### Почему Streamlit
HITL-форма с live-стримингом нужна была за один вечер, не за неделю.
...
|---|---|---|
| LangGraph orchestration | Peer-to-peer agent messaging | Аудитабельность важнее emergent behavior в incident response |
| Централизованный `IncidentState` | Per-agent memory / blackboard | Единственный source of truth; каждый узел знает только своё |
| Deterministic severity routing | LLM-router (severity решает LLM) | Предсказуемый cost; нельзя дать LLM «придумать» маршрут |
| `interrupt_before` HITL | Continuous autonomous execution | Compliance: инженер берёт ответственность за CRITICAL |
| Haiku для шаблонных узлов | Sonnet везде | Sonnet на Triage — переплата в 3–5x без прироста качества |
| ChromaDB embedded | Qdrant/Weaviate server | Нулевая инфраструктура для MVP; production-решение другое (см. ниже) |
| Синтетическая knowledge base | Реальные runbooks | Реальные данные требуют реальной компании; структура готова к замене |

---

## Сравнение с production-альтернативами

| | PagerDuty AIOps | AWS DevOps Guru | Этот проект |
|---|---|---|---|
| Подход | ML correlation, auto-grouping | ML anomaly detection | Orchestrated LLM workflow |
| Контроль / аудит | Vendor dashboard | AWS dashboard | Полный (open source, LangSmith trace) |
| HITL | Ограниченный | Нет | Явный interrupt_before |
| Стоимость | $$$ SaaS / host | $$ per resource/hour | $0.003–0.017 / инцидент |
| Кастомизация workflow | Ограничена | Нет | Полная (граф + промпты) |
| Knowledge base | Внешние интеграции | AWS-only | Custom RAG, любые источники |
| Vendor lock | PagerDuty | AWS | Нет |

**Позиционирование:** не конкурент enterprise-решениям по охвату, но демонстрирует
архитектурный подход, который enterprise-продукты не предлагают — полная прозрачность
и контролируемость каждого шага.
...
|---|---|---|---|
| Diagnosis Agent галлюцинирует root cause | Средняя (LLM) | HITL перед action'ом на CRITICAL | LLM-as-judge eval |
...
ui/
├── app.py                   # main page: streaming + HITL
...
Подводные камни, на которые ушло заметное время.

### `load_dotenv(override=True)` обязателен
Если в shell определена пустая `ANTHROPIC_API_KEY=`, Anthropic SDK не падает на
этапе импорта, но падает при первом вызове с невнятной ошибкой авторизации.
`override=True` гарантирует, что `.env` перезаписывает shell-переменные.
...
`graph.stream` при HITL-апруве, состояние сессии разнесено на 7 стадий
...
### Выброс latency: 83 секунды в реальных данных
Incident 3 (CRITICAL/data): Postmortem Agent занял 67 секунд вместо обычных 6–8.
Причина — медленный ответ LLM API (нет retry). Это реальные данные из
`data/metrics.jsonl`, не синтетика. В v1.5 планируется timeout + exponential backoff.
